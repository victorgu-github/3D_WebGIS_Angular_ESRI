(function () {
    ...
    multiselect.directive('multiselect', ['$rootScope', '$filter', '$document', '$log', function ($rootScope, $filter, 
        $document, $log) {
        return {
            restrict: 'AE',
            scope: {
            ...
            link: function ($scope, $element, $attrs, $ngModelCtrl) {
                ...
                var updateSelectionLists = function () {
                    if (!$ngModelCtrl.$viewValue) {
                        ...
                        if (Array.isArray($scope.resolvedOptions)) {
                ...
                $scope.getButtonText = function () {
                    //If $scope.selectOptions.length is 1, 2, 3, display the multicast addresses
                    if ($scope.selectedOptions && $scope.selectedOptions.length > 0 && $scope.selectedOptions.length <= 3) {
                        return $scope.getDisplay($scope.selectedOptions.join());
                    }
                    //If $scope.selectOptions.length is bigger than 3, only display how many options have been selected
                    if ($scope.selectedOptions && $scope.selectedOptions.length > 3) {
                    ...
                    //If none of option has been selected, display default place holder
                    } else {
                        return $scope.labels && $scope.labels.select ? $scope.labels.select : ($scope.placeholder || 'Select');
                    }
                };
                ...
                $rootScope.$on('updateButtonText', function () {
                    $scope.selectedOptions = [];
                });

                $rootScope.$on("initLoraDevMulticastAddrArrayForGenUsr", function(event, args){
                    $scope.selectedOptions = args.MulticastAddrArray;
                });
            }
        };
    }]);
...
angular.module("multiselect.html", []).run(["$templateCache", function ($templateCache) {
  $templateCache.put("multiselect.html",
    ...
    "    <button type=\"button\" class=\"btn dropdown-toggle\" ng-class=\"classesBtn\" ng-click=\"toggleDropdown()\" ng-disabled=\"disabled\" style=\"white-space: nowrap; overflow-x: hidden; text-overflow: ellipsis; text-align:left; padding-right:0px; border-top-left-radius:0px; border-bottom-left-radius: 0px\">\n" +
    "        {{getButtonText()}}&nbsp;<span class=\"caret pull-right\" style=\"margin-top:8px; margin-right:5px; border-left: 3px solid transparent; border-right: 3px solid transparent; border-top: 6px solid\"></span>\n" +
    "    </button>\n" +
    "    <ul class=\"dropdown-menu dropdown-menu-form\"\n" +
    "        ng-style=\"{display: open ? 'block' : 'none'}\" style=\"width: 100%; height: 250px; overflow-x: auto\">\n" +
    "\n" +
    ...
    "        <li role=\"presentation\" ng-repeat=\"option in resolvedOptions \"\n" +
    "            ng-class=\"{disabled : selectionLimit && selectedOptions.length >= selectionLimit}\">\n" +
    "            <div class=\"form-check\">\n" +
    "            <input type=\"checkbox\" class=\"form-check-input\" ng-checked=\"isSelected(option)\" ng-click=\"toggleItem(option); $event.stopPropagation()\" style=\"margin-left:10px\"></input>\n" +
    "            <a class=\"form-check-label\" href=\"\" title=\"{{showTooltip ? getDisplay(option) : ''}}\" ng-click=\"toggleItem(option); $event.stopPropagation()\" style=\"position:absolute; margin-left:10px; overflow-x: hidden;text-overflow: ellipsis; text-align:left; color:black\">\n" +
    "                {{getDisplay(option)}}\n" +
    "            </a>\n" +
    "            </div>\n" +
    "        </li>\n" +
