...
module.exports = function ($scope, $rootScope, $window, $routeParams, dashTableConfig, formValidator, AccountService, 
  loraDeviceService, dashboardSharedService, editSharedService, EditLoraDeviceService) {
  ...
  //Initialize the applicationIDs Array;
  ...
  $scope.updateSubTypes = function () {
    ...
    $scope.subTypes = EditLoraDeviceService.getSubTypes($scope.subTypeMap, $scope.editEntry);
  };
      ...
      if (validRefLatAndRefLonVal) {
        loraDeviceService.updateLoraDevice(editEntry).then(function (response) {
          if (response.status === "success") {
            ...
            $window.location.href = $scope.loraDeviceTableUrl;
          }
        ...
        $window.alert("Latitude and Longitude works together, if you want to clear Latitude and Longitude value, please set both of them to null ");
      }
    }
    else {
      $scope.formStatus = FORM_STATUS.INVALID;
  ...
  function initMulticastAddrArray() {
    ...
    dashboardSharedService.initMulticastAddrArrayForAllMode([$routeParams.applicationID]).then(function (resp) {
      ...
      dashboardSharedService.removeMulticastAddrNotExistInSpecificApplication($scope.multicastAddrArray, $scope.editEntry.MulticastAddrArray);
